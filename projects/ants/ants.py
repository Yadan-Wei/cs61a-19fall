...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        i = 0
        while i < self.min_range and cur_place:
            cur_place = cur_place.entrance
            i += 1
        while i < self.max_range and cur_place and not cur_place.bees:
            cur_place = cur_place.entrance
            i += 1 
        if not cur_place or cur_place.name == 'Hive':
            return None
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.is_queen:
            ...
            while cur_place:
                if cur_place.ant:
                    if not cur_place.ant.damage_doubled:
                        cur_place.ant.damage *= 2
                        cur_place.ant.damage_doubled = True
                    if cur_place.ant.is_container and cur_place.ant.contained_ant and not cur_place.ant.contained_ant.damage_doubled:
                        cur_place.ant.contained_ant.damage *= 2
                        cur_place.ant.contained_ant.damage_doubled = True
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        while cur_place is not beehive:
            if cur_place.ant and cur_place.ant != self:
                result[cur_place.ant] = distance
                if cur_place.ant.is_container and cur_place.ant.contained_ant:
                    result[cur_place.ant.contained_ant] = distance
            if cur_place.bees:

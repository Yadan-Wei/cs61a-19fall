...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    # if frist element is a special form, find the relevant do_?_form in env and elvaluate
    ...
    # call expression
        ...
        # elvaluate all operands
        operands = rest.map(lambda x: scheme_eval(x, env))
        # apply evaluated operator to evaluated operands
...
def scheme_apply(procedure, args, env):
    ...
    # if the procedure is a BuiltinProcedure, apply arguments
    if isinstance(procedure, BuiltinProcedure):
        return procedure.apply(args, env)
    # if the procedure is a user define one, create new frame, and call eval_all on its body
    else:
...
def eval_all(expressions, env, tail = False):
    ...
    return eval_all(expressions.rest, env, tail)  # replace this with lines of your own code
    # END PROBLEM 7
...
# tail call
# The last body sub-expression in a lambda expression: scheme_apply user define procedure part with tail = True
# Sub-expressions 2 & 3 in a tail context if expression: change do_if_form 
# All non-predicate sub-expressions in a tail context cond: change do_cond_form
# The last sub-expression in a tail context and or or: change do_and_form and do_or_form
# The last sub-expression in a tail context begin: change do_begin_form

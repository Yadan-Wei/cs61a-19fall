...
def add_trees(t1, t2):
    ...
    if is_leaf(t2):
        ...
        if len(t1) > len(t2):
            new_t2_branches = branches(t2) + [[0] for x in range(len_diff)]
            return tree(label(t1)+label(t2),[add_trees(t1_br,t2_br) for t1_br, t2_br in zip(branches(t1),new_t2_branches)])
        else:
            new_t1_branches = branches(t1) + [[0] for x in range(len_diff)]
            return tree(label(t1)+label(t2),[add_trees(t1_br,t2_br) for t1_br, t2_br in zip(new_t1_branches,branches(t2))])

...
def choose(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    para_new = [x for x in paragraphs if select(x)]
    if k >= len(para_new):
        return ''
    return para_new[k]
    # END PROBLEM 1
...
def about(topic):
    ...
    import utils
    def g(para):
        para_re = utils.remove_punctuation(para)
        para_re_spl = utils.split(para_re)
        for words in topic:
            for para_words in para_re_spl:
                if utils.lower(words) == utils.lower(para_words):
                    return True
        return False
    return g
...
def accuracy(typed, reference):
    ...
    # BEGIN PROBLEM 3
    "*** YOUR CODE HERE ***"
    type_len,refer_len = len(typed_words),len(reference_words)
    if type_len ==refer_len== 0:
        return 100.0
    elif type_len == 0:
        return 0.0
    else:
        count = 0
        for i in range(min(type_len,refer_len)):
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    for word in valid_words:
        diff.append([diff_function(typed_word,word,limit),word])
    diff_min = min(diff,key = lambda item:item[0])
    if diff_min[0] > limit:
        return typed_word
    return diff_min[1]
    # END PROBLEM 5
...
def time_per_word(times_per_player, words):
    ...
    for player in times_per_player:
        ...
        for i in range(1,len(player)):
            times_player.append(player[i]-player[i-1])
        times_real.append(times_player)

    return game(words,times_real)
...
def fastest_words(game):
    ...
    for i in word_indices:
        min,fastest_palyer = time(game,0,i),0
        for j in player_indices:
            if time(game,j,i) < min:
                min,fastest_palyer = time(game,j,i),j 
        result[fastest_palyer].append(word_at(game,i))
    return result
